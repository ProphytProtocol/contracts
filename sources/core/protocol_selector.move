...
/// AI-driven protocol selection with auto-deposit/withdraw and rebalancing logic
...
module prophyt::protocol_selector {
    ...
    public struct ProtocolRegistry<phantom CoinType> has key {
        ...
        protocol_states: Table<u8, ID>, // Maps protocol_type to state object ID
    ...
    public fun register_protocol<CoinType>(
        ...
        // Get protocol details based on type
    ...
    /// Select the best protocol using AI-driven scoring
    public fun select_best_protocol<CoinType>(
        ...
        // Update all protocol info before selection
    ...
    /// Helper function for recursive protocol selection
    fun select_best_loop<CoinType>(
        ...
        if (i < len) {
            ...
            let (new_best, new_score) = if (staking_protocol::is_active(protocol) &&
    ...
    /// Auto-deposit funds to the best protocol
    ...
    public fun auto_deposit<CoinType>(
        ...
        let success = if (protocol_type == constants::protocol_suilend()) {
            ...
            // Check whitelist for Haedal
            if (haedal_adapter::is_whitelisted(haedal_state, user_addr)) {
    ...
    /// Auto-withdraw funds from protocols
    public fun auto_withdraw<CoinType>(
        ...
        let user_addr = tx_context::sender(ctx);
        let len = vector::length(&registry.available_protocols);
        withdraw_from_protocols(registry, suilend_state, haedal_state, volo_state, amount, user_addr, 0, len, ctx)
    }

    /// Helper function for withdrawing from protocols
            ...
            if (staking_protocol::is_active(protocol)) {
                ...
                if (balance >= amount) {
                    ...
                    if (coin::value(&withdrawn_coin) > 0) {
                        ...
                        // Continue searching if nothing withdrawn
                        transfer::public_transfer(withdrawn_coin, user_addr);
                        withdraw_from_protocols(registry, suilend_state, haedal_state, volo_state, amount, user_addr, i + 1, len, ctx)
                    }
                } else {
                    withdraw_from_protocols(registry, suilend_state, haedal_state, volo_state, amount, user_addr, i + 1, len, ctx)
                }
    ...
    /// Get total balance across all protocols for a user
    ...
    /// Helper function to sum balances recursively
    fun sum_balances<CoinType>(
        ...
        if (i < len) {
            ...
            sum_balances(registry, user_addr, suilend_state, haedal_state, volo_state, new_total, i + 1, len)
        } else {
            total
        }
    }
    ...
    fun calculate_protocol_score(protocol: &ProtocolInfo): u64 {
        ...
        // TVL score (20% weight) - higher TVL = higher score
        ...
        // Risk score (20% weight) - lower risk = higher score
        let risk_score = ((11 - (risk as u64)) * constants::risk_weight()) / 10;

        apy_score + tvl_score + risk_score
    }

    /// Update all protocol information
    fun update_all_protocol_info<CoinType>(_registry: &mut ProtocolRegistry<CoinType>) {
        // In production, fetch real-time data from oracles or protocol APIs
        // For now, protocols are updated via their adapter update functions
    }

    /// Get protocol details based on type
    fun get_protocol_details(
        protocol_type: u8,
        _protocol_address: address
    ): (String, u64, u64) {
        if (protocol_type == constants::protocol_suilend()) {
            (suilend_adapter::get_protocol_name(), 500, 100000000) // 5% APY, mock TVL
        } else if (protocol_type == constants::protocol_haedal()) {
            (haedal_adapter::get_protocol_name(), 750, 80000000) // 7.5% APY, mock TVL
        } else if (protocol_type == constants::protocol_volo()) {
            (volo_adapter::get_protocol_name(), 650, 120000000) // 6.5% APY, mock TVL
        } else {
            (std::string::utf8(b"Unknown"), 0, 0)
        }
    }

    /// Get balance for a specific protocol
    ...
    /// Get current best APY across all protocols
    public fun get_current_best_apy<CoinType>(
        registry: &mut ProtocolRegistry<CoinType>
    ): u64 {
        update_all_protocol_info(registry);
        let len = vector::length(&registry.available_protocols);
    ...
    /// Helper function to find best APY recursively
    fun find_best_apy_loop(
