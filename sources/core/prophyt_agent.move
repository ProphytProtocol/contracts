...
module prophyt::prophyt_agent {
    ...
    public struct AgentConfig has key {
        ...
        rebalance_interval: u64, // epochs between rebalances
    ...
    /// Analyze and propose rebalancing opportunities
    public fun analyze_rebalance_opportunity<CoinType>(
        ...
        // Find highest and lowest APY protocols
        find_rebalance_opportunity(protocols, config, 0, len)
    }

    /// Helper function to find rebalancing opportunity
    fun find_rebalance_opportunity(
        ...
        if (i < len) {
            ...
            // Calculate APY difference in basis points
    ...
    /// Find protocol with highest APY
    fun find_best_protocol(
        protocols: &vector<ProtocolInfo>,
        _i: u64,
        len: u64
    ): ProtocolInfo {
    ...
    fun find_best_loop(
        ...
        if (i < len) {
            ...
            let new_best = if (staking_protocol::is_active(protocol) &&
    ...
    /// Find protocol with lowest APY
    fun find_worst_protocol(
        protocols: &vector<ProtocolInfo>,
        _i: u64,
        len: u64
    ): ProtocolInfo {
    ...
    fun find_worst_loop(
        ...
        if (i < len) {
            let protocol = vector::borrow(protocols, i);
            let new_worst = if (staking_protocol::is_active(protocol) &&
    ...
    /// Execute rebalancing based on proposal
    public fun execute_rebalance<CoinType>(
        ...
        event::emit(RebalanceExecuted {
            ...
            to_protocol: proposal.to_protocol,
            amount: proposal.amount,
            old_apy: 0, // Would fetch from protocol
