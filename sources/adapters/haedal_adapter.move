/// Haedal Adapter
/// Adapter for integrating with Haedal liquid staking protocol on Sui
...
module prophyt::haedal_adapter {
    ...
    /// Initialize Haedal adapter state
    ...
    /// Deposit coins into Haedal (liquid staking)
    ...
    /// Get user balance in Haedal
    public fun get_balance<CoinType>(
        state: &HaedalState<CoinType>,
        user_addr: address
    ): u64 {
    ...
    public fun is_whitelisted<CoinType>(
        state: &HaedalState<CoinType>,
        user_addr: address
    ): bool {
