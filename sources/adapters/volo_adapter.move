/// Volo Adapter
/// Adapter for integrating with Volo yield aggregation protocol on Sui
...
module prophyt::volo_adapter {
    ...
    public struct YieldStrategy has store, copy, drop {
        id: u64,
        name: String,
        target_apy: u64,
    ...
    public fun withdraw<CoinType>(
        ...
        *user_shares = *user_shares - shares;
        ...
        // In real integration, withdraw from Volo strategies
        coin::zero<CoinType>(_ctx)
    }

    /// Get user balance in Volo (in underlying tokens)
    public fun get_balance<CoinType>(
        state: &VoloState<CoinType>,
        user_addr: address
    ): u64 {
    ...
    /// Get all strategies
    public fun get_all_strategies<CoinType>(
        state: &VoloState<CoinType>
    ): &VecMap<u64, YieldStrategy> {
        &state.strategies
    }
